...
def analyze_face(frame):
    ...
    # If MediaPipe failed on Railway
    ...
    if results.multi_face_landmarks:
        ...
        right_eye_gap = abs(
            right_eye_top.y - right_eye_bottom.y
        )

        avg_eye_gap = (
            left_eye_gap + right_eye_gap
        ) / 2
        ...
        if (
            face_center_x < 0.42
            or face_center_x > 0.58
        ):
            target_eye_contact -= 30

        target_eye_contact = max(
            0,
            min(100, target_eye_contact)
        )

        left_face = face_landmarks.landmark[234]
        right_face = face_landmarks.landmark[454]

        posture_difference = abs(
            left_face.y - right_face.y
        )
    ...
    current_confidence = smooth_change(
        current_confidence,
        target_confidence
    )

    current_engagement = smooth_change(
        current_engagement,
        target_engagement
    )

    current_speech = smooth_change(
        current_speech,
        target_speech
    )
